...
uniform float u_malware;

out vec4 fragColor;

// --- 1. PROCEDURAL 3D NOISE ---
...
vec3 getEraserPos() {
    ...
    mouse_rd.yz *= rot(u_cameraRot.y); 
    mouse_rd.xz *= rot(u_cameraRot.x);
    vec3 ro = vec3(0.0, 0.0, -u_zoom);
    ro.yz *= rot(u_cameraRot.y); 
    ro.xz *= rot(u_cameraRot.x);
    return ro + mouse_rd * (u_zoom - 0.5);
}

// --- 3. SCENE MATH ---
float getBaseShape(vec3 pos) {
    float dynamicRipple = 0.1 * u_curse + (u_audioBass * 0.4);
    float ripple = sin(5.0 * pos.x + u_time * 2.0) * 
                   sin(5.0 * pos.y + u_time * 2.0) * 
                   sin(5.0 * pos.z + u_time * 2.0) * dynamicRipple;
    float currentRadius = 1.0 + (u_audioBass * 0.3);
    float sphereSDF = length(pos) - currentRadius; 
    float boxSDF = sdBox(pos, vec3(currentRadius * 0.8));
    return mix(sphereSDF, boxSDF, u_morph) - ripple;
}
...
float map(vec3 p) {
    ...
    float smoothShape = getBaseShape(objPos);
    float mutatedShape = smoothShape;

    // Malware Voxelizer
    if (u_malware > 0.0) {
        float vSize = 0.15;
        vec3 gridCenter = floor(objPos / vSize) * vSize + vSize * 0.5;
        float centerDist = getBaseShape(gridCenter);

        vec3 twitchOffset = vec3(0.0);
        float twitch = hash(gridCenter.y * 57.0 + floor(u_time * 12.0));
        
        if (twitch > 0.85) {
            twitchOffset.x = sin(u_time * 50.0 + gridCenter.y) * u_audioTreble * 3.0;
            twitchOffset.z = cos(u_time * 40.0 + gridCenter.x) * u_audioBass * 3.0;
        }

        vec3 localCube = objPos - (gridCenter + twitchOffset * u_malware);
        float cubeDist = sdBox(localCube, vec3(vSize * 0.45));
        float voxelShape = max(centerDist, cubeDist);
        
        mutatedShape = mix(smoothShape, voxelShape, u_malware);
    }
    
    float eraserSphere = length(p - getEraserPos()) - 0.5; 
    float corruptedShape = smax(mutatedShape, -eraserSphere, 0.3) * 0.3; 
...
vec3 getLight(vec3 p, vec3 rd) {
    ...
    if (isObject) {
        ...
        baseColor = mix(baseColor, vec3(0.0), corruptionBlend); 
...
void main() {
    ...
    vec3 rd_r = normalize(vec3(uv + glitchOffset, 1.0)); 
    rd_r.yz *= rot(u_cameraRot.y); rd_r.xz *= rot(u_cameraRot.x);
    
    vec3 rd_g = normalize(vec3(uv, 1.0)); 
    rd_g.yz *= rot(u_cameraRot.y); rd_g.xz *= rot(u_cameraRot.x);
    
    vec3 rd_b = normalize(vec3(uv - glitchOffset, 1.0)); 
    rd_b.yz *= rot(u_cameraRot.y); rd_b.xz *= rot(u_cameraRot.x);
    
    vec3 finalColor = vec3(renderScene(ro, rd_r).r, renderScene(ro, rd_g).g, renderScene(ro, rd_b).b);

    // Bright-Pass Bloom
    if (u_bloom > 0.0) {
        float bSize = 0.015 * u_bloom; 
        ...
        vec3 b1 = renderScene(ro, rd1); 
        vec3 b2 = renderScene(ro, rd2);
        vec3 b3 = renderScene(ro, rd3); 
        vec3 b4 = renderScene(ro, rd4);
